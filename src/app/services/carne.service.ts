...
export class CarneService {
  ...
  private getTimestamp(date: any): firebase.firestore.Timestamp {
    return date instanceof firebase.firestore.Timestamp ? date : (date?.toDate() as firebase.firestore.Timestamp);
  }
  ...
  async eliminarCorte(corte: Corte, tipoCarne: string): Promise<void> {
    try {
      ...
      if (carneDoc.exists()) {
        const updatedCortes = (carneDoc.data() as any)['cortes'].filter((c: Corte) => c.nombre !== corte.nombre);
