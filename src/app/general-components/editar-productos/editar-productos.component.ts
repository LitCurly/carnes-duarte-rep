...
export class EditarProductosComponent implements OnInit {
  ...
  actualizarCorte(): void {
    if (this.validarFormulario()) {
      this.carneService
        .actualizarCorte(this.tipoCarne, this.corteId, this.corte)
        .then(() => {
  ...
  validarFormulario(): boolean {
    return this.corte.nombre !== '' && this.corte.precioPorKilo > 0 && this.corte.stock >= 0
  }
