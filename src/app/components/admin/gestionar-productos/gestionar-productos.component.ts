...
export class GestionarProductosComponent implements OnInit {
  ...
  filterCortes(): void {
    ...
    if (this.searchQuery) {
      filteredCortes = filteredCortes.filter(corte => corte.nombre.toLowerCase().includes(this.searchQuery.toLowerCase()));
    }

    if (this.selectedPreparation === 'sin-preparaciones') {
      filteredCortes = filteredCortes.filter(corte => !corte.preparaciones || corte.preparaciones.length === 0);
    } else if (this.selectedPreparation && this.selectedPreparation !== 'all') {
      filteredCortes = filteredCortes.filter(corte => corte.preparaciones?.includes(this.selectedPreparation));
    }

    this.filteredCortes = filteredCortes;
    this.paginatedCortes = filteredCortes.slice((this.currentPage - 1) * this.itemsPerPage, this.currentPage * this.itemsPerPage);
  ...
  eliminarCorte(corte: CorteExtendido): void {
    if (confirm(`¿Seguro que deseas eliminar el corte '${corte.nombre}'?`)) {
      this.carneService.eliminarCorte(corte, corte.carne).then(() => {
